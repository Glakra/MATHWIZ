...
def format_fraction(numerator, denominator):
    """Create HTML for a properly formatted fraction"""
    return f'''
    <span style="display: inline-block; vertical-align: middle; text-align: center; margin: 0 3px;">
        <span style="display: block; border-bottom: 1px solid black; padding: 0 4px;">{numerator}</span>
        <span style="display: block; padding: 0 4px;">{denominator}</span>
    </span>
    '''

def format_mixed_number(whole, numerator, denominator):
    """Create HTML for a properly formatted mixed number"""
    return f'''
    <span style="font-size: 28px;">
        {whole}
        <span style="display: inline-block; vertical-align: middle; text-align: center; margin: 0 5px;">
            <span style="display: block; border-bottom: 1px solid black; padding: 0 4px; line-height: 0.8;">{numerator}</span>
            <span style="display: block; padding: 0 4px; line-height: 0.8;">{denominator}</span>
        </span>
    </span>
    '''
...
def display_problem():
    ...
    # Create the equation display with properly formatted fractions
    mixed1_html = format_mixed_number(data['whole1'], data['num1'], data['denominator'])
    mixed2_html = format_mixed_number(data['whole2'], data['num2'], data['denominator'])
    operator = "+" if data['operation'] == "add" else "−"
    ...
    st.markdown(
        ...
        <div style="font-size: 28px; font-weight: bold; margin: 30px 0; 
            ...
            {mixed1_html} {operator} {mixed2_html} = 
...
def show_feedback():
    ...
    if is_correct:
        ...
        if isinstance(correct_answer, int):
            answer_html = str(correct_answer)
        else:
            # Parse the answer string to get components
            parts = st.session_state.correct_answer.split()
            if len(parts) == 1:
                # Just a fraction
                if '/' in parts[0]:
                    n, d = parts[0].split('/')
                    answer_html = format_fraction(n, d)
                else:
                    answer_html = parts[0]
            else:
                # Mixed number
                whole = parts[0]
                n, d = parts[1].split('/')
                answer_html = format_mixed_number(whole, n, d)
        
        st.markdown(
            f"""
            <div style="font-size: 24px; font-weight: bold; color: green; text-align: center;">
                ✅ {mixed1_html} {operator} {mixed2_html} = {answer_html}
            </div>
            """,
            unsafe_allow_html=True
        )
    ...
    with st.expander("📖 **See Step-by-Step Solution**", expanded=True):
        if data["operation"] == "add":
            ...
            **Step 2: Add the fractions**
            ...
            # Display fraction addition with proper formatting
            frac1_html = format_fraction(data['num1'], data['denominator'])
            frac2_html = format_fraction(data['num2'], data['denominator'])
            sum_html = format_fraction(data['num1'] + data['num2'], data['denominator'])
            
            st.markdown(
                f"<div style='font-size: 20px; margin: 10px 0;'>{frac1_html} + {frac2_html} = {sum_html}</div>",
                unsafe_allow_html=True
            )
            
            st.markdown("**Step 3: Simplify if needed**")
            ...
            if data['num1'] >= data['denominator']:
                ...
                **Step 2: Subtract the fractions**
                ...
                # Display fraction subtraction with proper formatting
                frac1_html = format_fraction(data['num1'], data['denominator'])
                frac2_html = format_fraction(data['num2'], data['denominator'])
                diff_html = format_fraction(data['num1'] - data['num2'], data['denominator'])
                
                st.markdown(
                    f"<div style='font-size: 20px; margin: 10px 0;'>{frac1_html} − {frac2_html} = {diff_html}</div>",
                    unsafe_allow_html=True
                )
